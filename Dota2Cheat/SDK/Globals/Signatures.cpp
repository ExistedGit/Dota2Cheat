...
#define SET_VAR(var, data) var = (data); LogF(LP_DATA, "{}: {}", #var, (void*)var);
...
void Signatures::FindSignatures() {
	...
	SET_VAR(GetPlayer, ssctx.Scan("33 C0 83 F9 FF", L"client.dll"));

	// xref "particles/ui_mouseactions/waypoint_flag.vpcf"
	// The one with the bigger offset from function
	SET_VAR(PrepareUnitOrders, ssctx.Scan("E8 ? ? ? ? 49 8B CF FF C5", L"client.dll").GetAbsoluteAddress(1, 5));
	...
	SET_VAR(CDOTAParticleManager::DestroyParticleFunc, ssctx.Scan("E8 ? ? ? ? 48 83 C3 1C", L"client.dll").GetAbsoluteAddress(1, 5));

	// JS function
	SET_VAR(CDOTAGameRules::GetGameTimeFunc, ssctx.Scan("E8 ? ? ? ? 8B 04 2E", L"client.dll").GetAbsoluteAddress(1, 5));

	// UnknownCheats wiki -> Dota 2 -> link to Using engine functions
	SET_VAR(WorldToScreen, ssctx.Scan("E8 ? ? ? ? 83 7D B7 00", L"client.dll").GetAbsoluteAddress(1, 5));

	// xref: "BuildCacheSubscribed(CEconItem)"
	SET_VAR(CreateEconItem, ssctx.Scan("48 83 EC ? B9 ? ? ? ? E8 ? ? ? ? 48 85 C0 74 ? 48 8D 0D", L"client.dll"));
	// Last call in JS func GetLevelSpecialValueFor for a method of CDOTABaseAbility
	SET_VAR(CDOTABaseAbility::GetLevelSpecialValueForFunc, ssctx.Scan("E8 ? ? ? ? F3 0F 5A C0", L"client.dll").GetAbsoluteAddress(1, 5));
	...
	SET_VAR(DispatchPacket, ssctx.Scan("E8 ? ? ? ? 48 8B CF E8 ? ? ? ? B8", L"client.dll").GetAbsoluteAddress(1, 5));

	//xref: "CProtoBufMsg::BAsyncSendProto"
	SET_VAR(BAsyncSendProto, ssctx.Scan("E8 ? ? ? ? 48 8B 4D 90 48 89 7C 24", L"client.dll").GetAbsoluteAddress(1, 5));
	SET_VAR(GetItemSchema, ssctx.Scan("E8 ? ? ? ? 48 89 5D 18", L"client.dll").GetAbsoluteAddress(1, 5));
	SET_VAR(CDOTAItemSchema::GetItemDefByIndex, ssctx.Scan("E8 ? ? ? ? 83 FE 05", L"client.dll").GetAbsoluteAddress(1, 5));
	...
	SET_VAR(CDOTARichPresence::SetRPStatusFunc, ssctx.Scan("4C 89 4C 24 20 55 53 57 41 54", L"client.dll"));
	...
	SET_VAR(SaveSerializedSOCache, ssctx.Scan("4C 8B DC 57 48 81 EC ? ? ? ? 48 8B 05", L"client.dll"));

	//xref: "#DOTA_GlobalItems_Emoticons"
	//decompile in IDA
	//in the middle, 240 lines in is:
	// if(!sub_XXXXXXX(v34, v89)) goto LABEL_XX:
	SET_VAR(BIsEmoticonUnlocked, ssctx.Scan("E8 ? ? ? ? 84 C0 0F 84 ? ? ? ? 48 8B 45 08", L"client.dll").GetAbsoluteAddress(1, 5));

	//xref "OnColorChanged", lea rax, [XXXXXXXXX] below it
	SET_VAR(CBaseEntity::OnColorChanged, ssctx.Scan("40 53 48 83 EC 20 48 8B D9 48 8B 89 ? ? ? ? 48 8B 01 0F B6 93", L"client.dll"));
