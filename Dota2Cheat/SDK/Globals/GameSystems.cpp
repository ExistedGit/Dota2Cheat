...
void GameSystems::FindGameSystems() {
	...
	//console command ^
	SET_VAR(GameUI, ssctx.Scan("E8 ? ? ? ? 48 8B C8 E8 ? ? ? ? 48 8B 3D", L"client.dll")
		.GetAbsoluteAddress(1)
		.GetAbsoluteAddress(3));
	...
	SET_VAR(ProjectileManagerPtr,
		ssctx.Scan("40 55 53 41 57 48 8D 6C 24 D0 48 81 EC 30 01 00 00 48 89 BC 24 58 01 00 00", L"client.dll")
		.Offset(0x95e)
		.GetAbsoluteAddress(3, 7));

	SET_VAR(GameRulesPtr, Address(CDOTAGameRules::GetGameTimeFunc).Offset(0xF).GetAbsoluteAddress(3, 7));
	
	SET_VAR(PlayerResourcePtr, ssctx.Scan("48 8B ? ? ? ? ? 48 85 C9 0F 85 ? ? ? ? B8 FF FF FF FF C3", L"client.dll")
		.GetAbsoluteAddress(3, 7));

	// xref: "flOffset"
	// lea rcx, [XXXXXXXXX] above it is GetParticleManager()
	SET_VAR(ParticleManagerPtr, ssctx.Scan("E8 ? ? ? ? 8B 14 9F", L"client.dll")
