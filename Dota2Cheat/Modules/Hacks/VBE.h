...
namespace Hacks {
	...
	private:
		...
		struct TrackingInfo {
			...
			float sscSum = 0;
			bool visible = false;
			ParticleWrapper vbeParticleWrap{};
		};
		std::map<CDOTABaseNPC*, TrackingInfo> TrackedEntities{};
	public:
		void Reset() {
			for (auto& [ent, info] : TrackedEntities)
				if (info.vbeParticleWrap.particle)
					GameSystems::ParticleManager->DestroyParticle(info.vbeParticleWrap);
			//TrackedEntities.clear();
		}
		void SubscribeEntity(CDOTABaseNPC* ent) {
			//TrackedEntities[ent] = TrackingInfo{};
		}
		void CreateVbeParticleFor(CDOTABaseNPC* ent) {
			TrackedEntities[ent].vbeParticleWrap = GameSystems::ParticleManager->CreateParticle(
		...
		void FrameBasedLogic() {
			for (auto& pair : TrackedEntities) {
				...
				if (pair.second.visible && !vbeParticleActive && Config::VBEShowParticle)
					...
					GameSystems::ParticleManager->DestroyParticle(trackInfo.vbeParticleWrap);
